  ...
  <div class="users">
    <div class="user-table">
        ...
        <table>
          ...
          <tbody>
            <tr v-for="user in users" :key="user.id">
              ...
              <td>
                <p v-if="user.status === 'active'" class="status active">
                  Active
                </p>
                <p v-else class="status disabled">Disabled</p>
              </td>
              <td>
                <div class="action">
                  <button id="edit" @click="routeTo(user.id)">Edit</button>
                  <button id="delete" @click="OpenDeleteUser(user.id)">
                    Delete
                  </button>
                </div>
              </td>
      ...
      <div v-else class="empty-users">
        <img src="../assets/empty.svg" alt="No User" />
        <h1>No User Found. Add New User</h1>
      </div>
              ...
              <div class="add-user-modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <button
                      class="btn"
                      style="background-color: #3c4b64; color:#ffffff"
                      @click="CloseAddUser"
                    >
                      Close
                    </button>
                  ...
                  <div class="btn-div">
                    <button class="addUser" @click="AddUserPost">
                      Add User
                    </button>
            ...
            <div class="modal-wrapper">
              <div class="modal-dialog">
                <div class="modal-content">
                  ...
                  <div class="btn-div">
                    <button class="accept" @click="deleteSelectedUSer">
                      Yes, Delete
                    </button>
                    <button class="reject" @click="deleteModel = false">
                      Cancel
                    </button>
                  </div>
                </div>
...
export default {
  ...
  methods: {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (response.status == 200 || response.status == 201) {
              //  Make get Request for updated Values
              this.getUsers();

              //  Clear the input values and close the modal
              this.CloseAddUser();
            }
            resolve(response);
    ...
    deleteSelectedUSer() {
      return new Promise((resolve, reject) => {
        axios
          .delete(`http://localhost:3000/users/${this.selectedUserId}`)
          .then((response) => {
            this.getUsers();
